...
export class UpdateModelCommand extends BaseUpdateModelCommand {
    protected override computeAnimation(
        ...
        forEachMatch(matchResult, (id, match) => {
            if (match.left !== undefined && match.right !== undefined) {
                const animation = this._updateElement(match.left as SModelElementImpl, match.right as SModelElementImpl);
     ...
     */
    private _updateElement(left: SModelElementImpl, right: SModelElementImpl): ElmentLinearInterpolationAnimation | undefined {
